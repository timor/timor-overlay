{ stdenv, lib, fetchurl, mfcl8650cdwlpr, makeWrapper, dpkg, pkgsi686Linux, coreutils }:
...
stdenv.mkDerivation rec {
  ...
  printer_model = model;
  printer_name = lib.toUpper model;
  ...
  binPath = lib.makeBinPath [ coreutils ];
  ...
  installPhase = ''
    ...
    wrapProgram $out/opt/brother/Printers/${model}/cupswrapper/brcupsconfpt1 \
      --prefix PATH ":" ${ lib.makeBinPath [ mfcl8650cdwlpr ] }
